...
Token* getToken(void) {
  ...
  case CHAR_EQ:
    ...
    readChar();
    return token;
  case CHAR_COMMA: // Dau phay
    token = makeToken(SB_COMMA, lineNo, colNo);
    readChar();
    return token;
  case CHAR_EXCLAIMATION: // Dau cham than
    ln = lineNo;
    cn = colNo;
    readChar();
    if(currentChar != EOF && charCodes[currentChar] == CHAR_EQ){
  ...
  case CHAR_SEMICOLON: // Dau cham phay
