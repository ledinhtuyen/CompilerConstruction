...
Token* readNumber(void) {
  ...
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)) {
    if(charCodes[currentChar] == CHAR_PERIOD && countPeriod != 0){
      printf("%d-%d: invalid double value\n", token->lineNo, token->colNo);
      exit(0);
		}

    if(charCodes[currentChar] == CHAR_PERIOD && charCodes[readChar()] == CHAR_DIGIT){
      countPeriod = 1;
...
Token* readString(void) {
  ...
  while (charCodes[currentChar] != CHAR_DOUBLEQUOTE) {
    if (currentChar == EOF) {
      token->tokenType = TK_NONE;
      printf("%d-%d: Error String", token->lineNo, token->colNo);
      return token;
    }
    else{
      if (count >= MAX_IDENT_LEN)
