...
{
  Type *expType;
  Type **lvalueType = (Type **)malloc(sizeof(Type *));
  int counterLValue = 1;
  int counterRValue = 1;
  ...
  while (lookAhead->tokenType == SB_COMMA)
  {
    ...
    counterLValue++;
    lvalueType = (Type **)realloc(lvalueType, sizeof(Type *) * counterLValue);
    lvalueType[counterLValue - 1] = compileLValue();
  }

  eat(SB_ASSIGN); // An Dau '='
  expType = compileExpression();
  checkTypeEquality(lvalueType[counterRValue - 1], expType);

  while (lookAhead->tokenType == SB_COMMA)
  {
    eat(SB_COMMA);
    counterRValue++;
    if (counterLValue < counterRValue)
    {
      printf("%d-%d: Extra RValue\n", lookAhead->lineNo, lookAhead->colNo);
      exit(0);
    }
    expType = compileExpression();
    checkTypeEquality(lvalueType[counterRValue - 1], expType);
...
Type *compileExpression3(void)
{
  ...
  case SB_PLUS:
    ...
    else if (type1 == doubleType)
    {
      checkDoubleType(type1);
    }
    type2 = compileExpression3();
    if (type2 != NULL)
    {
